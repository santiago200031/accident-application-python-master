...
def test_fetch_record_at_index_out_of_bounds():
    ...
    records_list = ["record1", "record2", "record3"]
    target_index = 5
    assert controller.fetch_record_at_index(records_list, target_index) == "Record not found"
...
def test_run_method_with_single_record():
    controller = IndexErrorIncident()
    assert controller.run() == "Record not found"