# json-decode-get_segments_by_document

**Exception:** `json.decoder.JSONDecodeError` — unhandled `await req.json()` in `main.py:2216` (`get_segments_by_document_http`).
...
## WireMock stub (mount in `mappings/json-decode-get_segments_by_document.json`)
```json
{
  "request": {
    "method": "POST",
