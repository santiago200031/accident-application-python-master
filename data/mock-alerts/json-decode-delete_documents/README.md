# json-decode-delete_documents

**Exception:** `json.decoder.JSONDecodeError` — unhandled `await req.json()` in `main.py:1666` (`delete_documents_http`).
...
## WireMock stub (mount in `mappings/json-decode-delete_documents.json`)
```json
{
  "request": {
    "method": "POST",
