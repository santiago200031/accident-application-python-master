# json-decode-update_segment

**Exception:** `json.decoder.JSONDecodeError` — unhandled `await req.json()` in `main.py:1694` (`update_segment_http`).
...
## WireMock stub (mount in `mappings/json-decode-update_segment.json`)
```json
{
  "request": {
    "method": "POST",
