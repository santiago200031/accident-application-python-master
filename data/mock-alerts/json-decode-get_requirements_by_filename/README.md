# json-decode-get_requirements_by_filename

**Exception:** `json.decoder.JSONDecodeError` — unhandled `await req.json()` in `main.py:2164` (`get_requirements_by_filename_http`).
...
## WireMock stub (mount in `mappings/json-decode-get_requirements_by_filename.json`)
```json
{
  "request": {
    "method": "POST",
